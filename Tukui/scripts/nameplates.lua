...
local updatePlate = function(self)
	...
	self.level:SetPoint("RIGHT", self.healthBar, "RIGHT", TukuiDB.Scale(-2), TukuiDB.mult)
...
local createPlate = function(frame)
	...
	if TukuiCF["nameplate"].showhealth == true then
		...
		healthBar.percent:SetPoint("CENTER", healthBar, "CENTER", 0, TukuiDB.mult)
	...
	if TukuiCF["nameplate"].showclassicons == true then
